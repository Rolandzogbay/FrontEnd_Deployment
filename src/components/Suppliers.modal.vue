          ...
          <div class="flex items-start justify-between gap-4">
            <div class="flex items-center gap-3 sm:gap-4 min-w-0">
              <div
                ...
                <i :class="isEditMode ? 'fa-solid fa-truck-field-un' : 'fa-solid fa-plus'"></i>
              </div>
              ...
              <div class="min-w-0">
                ...
                <p class="text-xs sm:text-sm text-slate-500 mt-1">
                  {{ isEditMode ?
                    "Update supplier information and keep your sourcing data accurate."
                    :
                    "Create a new supplier record for procurement and stock operations." }}
                </p>
              </div>
            </div>
          ...
          <form class="space-y-5" @submit.prevent="handleSubmit">
            <section class="rounded-[24px] border border-slate-200 bg-white p-4 sm:p-5">
              <div class="mb-4">
                ...
                <p class="text-sm text-slate-500 mt-1">Set up the main supplier details and primary contact information.
                </p>
              </div>
              ...
              <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div class="sm:col-span-2">
                  ...
                  <input v-model="form.name" type="text" placeholder="Enter supplier name"
                    ...
                    :class="errors.name ? 'border-red-300 focus:ring-red-500 bg-red-50/50' : ''" />
                ...
                <div>
                  ...
                  <input v-model="form.email" type="email" placeholder="supplier@email.com"
                    class="w-full px-4 py-3.5 rounded-2xl border border-slate-200 bg-slate-50/70 focus:bg-white focus:outline-none focus:ring-2 focus:ring-orange-500 text-sm sm:text-base transition"
                    :class="errors.email ? 'border-red-300 focus:ring-red-500 bg-red-50/50' : ''" />
                  <p v-if="errors.email" class="text-xs text-red-600 mt-2">{{ errors.email }}</p>
                </div>

                <div>
                  <label class="block text-sm font-semibold text-slate-700 mb-2">Phone Number</label>
                  <input v-model="form.phone_number" type="text" placeholder="+231..."
                    class="w-full px-4 py-3.5 rounded-2xl border border-slate-200 bg-slate-50/70 focus:bg-white focus:outline-none focus:ring-2 focus:ring-orange-500 text-sm sm:text-base transition"
                    :class="errors.phone_number ? 'border-red-300 focus:ring-red-500 bg-red-50/50' : ''" />
                ...
                <div class="sm:col-span-2">
                  ...
                  <input v-model="form.contact_person" type="text" placeholder="Enter primary contact person"
                    class="w-full px-4 py-3.5 rounded-2xl border border-slate-200 bg-slate-50/70 focus:bg-white focus:outline-none focus:ring-2 focus:ring-orange-500 text-sm sm:text-base transition" />
                </div>
              </div>
            </section>
            ...
            <section class="rounded-[24px] border border-slate-200 bg-white p-4 sm:p-5">
              <div class="mb-4">
                ...
                <p class="text-sm text-slate-500 mt-1">Add sourcing notes and location details for better operations.
                </p>
              </div>
              ...
              <div class="grid grid-cols-1 gap-4">
                <div>
                  ...
                  <textarea v-model="form.address" rows="3" placeholder="Enter supplier address"
                    class="w-full px-4 py-3.5 rounded-2xl border border-slate-200 bg-slate-50/70 focus:bg-white focus:outline-none focus:ring-2 focus:ring-orange-500 text-sm sm:text-base transition resize-none"></textarea>
                </div>

                <div>
                  <label class="block text-sm font-semibold text-slate-700 mb-2">Notes</label>
                  <textarea v-model="form.notes" rows="4" placeholder="Add notes about this supplier"
                    class="w-full px-4 py-3.5 rounded-2xl border border-slate-200 bg-slate-50/70 focus:bg-white focus:outline-none focus:ring-2 focus:ring-orange-500 text-sm sm:text-base transition resize-none"></textarea>
                </div>
              </div>
            </section>
          ...
          <div class="flex flex-col-reverse sm:flex-row sm:items-center sm:justify-end gap-3">
            ...
            <button type="button" @click="handleSubmit"
              ...
              <span v-if="!submitting">
                <i :class="isEditMode ? 'fa-solid fa-floppy-disk' : 'fa-solid fa-plus'" class="mr-2"></i>
...
const handleSubmit = async () => {
  ...
  try {
    emit("submit", {
      ...
      contact_person: String(form.value.contact_person || "").trim(),
