  ...
  <div v-if="open"
    ...
    <div
      ...
      <div
        ...
        <div class="min-w-0">
          <h3 class="text-xl sm:text-2xl font-bold text-slate-900">Sale Details</h3>
        ...
        <div class="flex items-center gap-2 shrink-0">
          <button v-if="sale" @click="$emit('print', sale)"
      ...
      <div v-else-if="sale" class="flex-1 overflow-y-auto p-4 sm:p-5 md:p-6 space-y-5 sm:space-y-6 bg-slate-50">
        <section
          class="rounded-[24px] sm:rounded-[28px] border border-slate-200 bg-gradient-to-r from-slate-900 via-slate-800 to-slate-900 p-4 sm:p-6 text-white overflow-hidden">
              ...
              <div class="flex items-start gap-4">
                ...
                <div class="min-w-0">
                  ...
                  <p class="text-slate-300 mt-2 max-w-2xl text-sm sm:text-base">
                    Detailed transaction summary including customer, sold items, payment method, and receipt
                    information.
                  </p>
                </div>
              </div>
        ...
        <section class="overflow-hidden rounded-3xl border border-slate-200 bg-white">
            ...
            <table class="min-w-full">
              <thead class="bg-white">
                <tr class="text-left text-sm text-slate-600">
                  ...
                  <th class="px-4 py-4 font-semibold">Product Code</th>
                  <th class="px-4 py-4 font-semibold">Barcode</th>
              ...
              <tbody>
                <tr v-for="item in sale.items" :key="item.id || `${item.productId}-${item.name}`"
          ...
          <div v-if="sale.items?.length" class="md:hidden p-4 space-y-3">
            <div v-for="item in sale.items" :key="item.id || `${item.productId}-${item.name}`"
              ...
              <div class="mt-2 space-y-1 text-xs text-slate-500">
                <p><span class="font-medium text-slate-700">Product Code:</span>
                  {{ item.sku || item.product?.sku || "Noproduct code" }}
                </p>
      ...
      <div v-if="sale" class="sm:hidden border-t border-slate-100 bg-white px-4 py-3">
...
const businessInfo = computed(() => {
  ...
  return {
    ...
    email: business.email || business.business_email || props.receiptBusiness?.email || "",
    phone:
      business.phone ||
